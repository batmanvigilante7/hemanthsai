...
function SplitFlipCard({ signal, index, progress, onOpen }: { signal: IdentitySignal; index: number; progress: MotionValue<number>; onOpen: () => void }) {
  const x = useTransform(progress, [0, 0.18, 0.22, 0.36, 0.80, 0.88, 0.97, 1], [joinedX[index], joinedX[index], joinedX[index], tableX[index], tableX[index], tableX[index], joinedX[index], joinedX[index]]);
  const openSafeY = 72;
  const y = useTransform(progress, [0, 0.18, 0.22, 0.36, 0.80, 0.88, 0.97, 1], [0, 0, 0, tableY[index] + openSafeY, tableY[index] + openSafeY, tableY[index] + openSafeY, 0, 0]);
  const rotateZ = useTransform(progress, [0, 0.18, 0.22, 0.36, 0.80, 0.88, 0.97, 1], [0, 0, 0, tableRotateZ[index], tableRotateZ[index], tableRotateZ[index], 0, 0]);
  const rotateY = useTransform(progress, [0, 0.36, 0.44, 0.80, 0.88, 1], [0, 0, 180, 180, 360, 360]);
  const scale = useTransform(progress, [0, 0.36, 0.80, 1], [1, 0.94, 0.94, 1]);
  const pointerEvents = useTransform(progress, (value) => value > 0.44 && value < 0.80 ? 'auto' : 'none');

  const joinedRadius = index === 0 ? '32px 0px 0px 32px' : index === 3 ? '0px 32px 32px 0px' : '0px 0px 0px 0px';
  const radius = useTransform(progress, [0, 0.06, 0.36, 0.88, 0.97, 1], [joinedRadius, joinedRadius, '32px 32px 32px 32px', '32px 32px 32px 32px', joinedRadius, joinedRadius]);
  const seamOpacity = useTransform(progress, [0, 0.06, 0.18, 0.88, 0.97, 1], [0, 0, 0.45, 0.45, 0, 0]);
  const border = useTransform(progress, [0, 0.06, 0.18, 0.88, 0.97, 1], ['1px solid rgba(255, 255, 255, 0)', '1px solid rgba(255, 255, 255, 0)', '1px solid rgba(255, 255, 255, 0.14)', '1px solid rgba(255, 255, 255, 0.14)', '1px solid rgba(255, 255, 255, 0)', '1px solid rgba(255, 255, 255, 0)']);
  const backBorder = useTransform(progress, [0, 0.06, 0.18, 0.88, 0.97, 1], ['1px solid rgba(255, 255, 255, 0)', '1px solid rgba(255, 255, 255, 0)', '1px solid rgba(255, 255, 255, 0.12)', '1px solid rgba(255, 255, 255, 0.12)', '1px solid rgba(255, 255, 255, 0)', '1px solid rgba(255, 255, 255, 0)']);
  const shadow = useTransform(progress, [0, 0.06, 0.36, 0.88, 0.97, 1], ['0px 0px 0px rgba(0,0,0,0)', '0px 0px 0px rgba(0,0,0,0)', '0px 38px 130px rgba(0,0,0,0.62)', '0px 38px 130px rgba(0,0,0,0.62)', '0px 0px 0px rgba(0,0,0,0)', '0px 0px 0px rgba(0,0,0,0)']);
...
function MobileStickyStack({ onOpen }: { onOpen: (index: number) => void }) {
  ...
  return <div ref={ref} className="relative px-5 py-16 md:hidden"><div className="mx-auto max-w-md"><FadeIn><p className="mb-3 text-[10px] font-black uppercase tracking-[0.28em] text-white/45">Identity</p><h2 className="text-[clamp(2.7rem,15vw,4.4rem)] font-black uppercase leading-[0.84] tracking-[-0.09em] text-white">The pattern behind the proof.</h2><p className="mt-5 text-base font-light leading-relaxed text-white/70">Before I became interested in AI, software, and product building, I was shaped by structure, discipline, voice, and execution.</p></FadeIn><div className="mt-10"><CinematicSplitIntroCard mobile /></div><div className="mt-8 pb-[70svh]">{identitySignals.map((signal, index) => <MobileStackCard key={signal.title} signal={signal} index={index} progress={scrollYProgress} onOpen={() => onOpen(index)} />)}</div></div></div>;
}

function DesktopIdentityShowcase({ onOpen }: { onOpen: (index: number) => void }) {
  return <div className="relative hidden px-5 py-10 sm:px-8 sm:py-14 md:block md:px-10"><div className="relative mx-auto max-w-[1680px]"><FadeIn><p className="mb-3 text-xs font-black uppercase tracking-[0.32em] text-white/45">Identity</p><h2 className="max-w-6xl text-[clamp(2.7rem,9.2vw,6rem)] font-black uppercase leading-[0.84] tracking-[-0.09em] text-white">The pattern behind the proof.</h2><p className="mt-5 max-w-4xl text-xl font-light leading-relaxed text-white/72">Before I became interested in AI, software, and product building, I was shaped by structure, discipline, voice, and execution.</p><p className="mt-4 max-w-4xl text-base leading-relaxed text-white/56">The environments changed — training grounds, uniforms, stages, classrooms, and laptops — but the pattern stayed the same: learn under pressure, communicate with clarity, and turn intent into visible work.</p></FadeIn><div className="mt-6"><CinematicSplitIntroCard onOpen={onOpen} /></div></div></div>;
}

export default function IdentityStack() {
  const [activeSignal, setActiveSignal] = useState<number | null>(null);
  return <section id="identity" className="relative overflow-x-clip bg-[#070707] text-white"><div className="absolute inset-0 bg-[radial-gradient(circle_at_12%_18%,rgba(255,255,255,.075),transparent_32%),radial-gradient(circle_at_86%_8%,rgba(141,162,255,.10),transparent_30%),linear-gradient(180deg,#070707,#050505)] pointer-events-none" /><DesktopIdentityShowcase onOpen={setActiveSignal} /><MobileStickyStack onOpen={setActiveSignal} /><AnimatePresence>{activeSignal !== null && <IdentitySignalModal signal={identitySignals[activeSignal]} onClose={() => setActiveSignal(null)} />}</AnimatePresence></section>;
}
