...
const projects = [
  ['01', 'Proof Hub', 'In progress', 'This website is my first public attempt at turning scattered interests, experiences, notes, and experiments into one visible identity.', ['Portfolio', 'Identity', 'Proof']],
  ['02', 'AI Workflow Lab', 'Exploring', 'Experiments with AI tools, automations, agents, and workflows that can become practical leverage instead of just curiosity.', ['AI', 'Automation', 'Systems']],
  ['03', 'UX Field Notes', 'Collecting', 'A growing collection of interface observations, product thinking, design breakdowns, and notes from learning how better digital experiences are made.', ['UX', 'Writing', 'Product']],
] as const;
...
function FadeIn({ children, delay = 0, y = 24, className = '' }: { children: ReactNode; delay?: number; y?: number; className?: string }) {
  return (
    <motion.div initial={{ opacity: 0, y }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-40px' }} transition={{ duration: 0.65, delay, ease: [0.22, 1, 0.36, 1] }} className={className}>
      {children}
    </motion.div>
  );
}
...
function ImageFrame({ src, alt, label, className = '', imgClassName = '', priority = false, fit = 'cover', tone = 'natural' }: { src: string; alt: string; label: string; className?: string; imgClassName?: string; priority?: boolean; fit?: 'cover' | 'contain'; tone?: 'natural' | 'soft' | 'mono' }) {
  ...
  return (
    <div className={`noise group relative overflow-hidden bg-white/[0.04] ${className}`}>
      {!broken ? <img src={src} alt={alt} onError={() => setBroken(true)} loading={priority ? 'eager' : 'lazy'} className={`h-full w-full ${fitClass} ${toneClass} transition-transform duration-700 group-hover:scale-[1.018] md:duration-1000 md:group-hover:scale-[1.035] ${imgClassName}`} /> : <div className="flex h-full min-h-[220px] w-full items-center justify-center bg-[linear-gradient(135deg,#171717,#050505)] px-5 text-center"><span className="rounded-full border border-white/10 px-5 py-3 text-[10px] uppercase tracking-[0.24em] text-white/55 sm:text-xs sm:tracking-[0.35em]">{label}</span></div>}
...
function Hero() {
  ...
  return (
    <section id="arrival" className="noise relative min-h-[100svh] overflow-hidden bg-[#050505] text-white">
      ...
      <nav className="fixed left-0 right-0 top-0 z-40 px-3 py-3 sm:px-6 sm:py-4"><div className="liquid-glass mx-auto flex max-w-7xl items-center justify-between rounded-full px-4 py-2.5 sm:px-5 sm:py-3"><a href="#arrival" className="max-w-[58vw] truncate text-sm font-black uppercase tracking-[-0.04em] sm:text-base">Hemanth Sai<span className="text-white/50">.</span></a><div className="hidden items-center gap-7 lg:flex">{navLinks.map(([label, href]) => <a key={label} href={href} className="text-xs font-medium uppercase tracking-[0.24em] text-white/65 transition hover:text-white">{label}</a>)}</div><button onClick={() => setOpen(!open)} className="grid h-10 w-10 place-items-center rounded-full text-white lg:hidden" aria-label="Menu">{open ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}</button><div className="hidden lg:block"><Button href="mailto:hemanthsairoyal7@gmail.com">Connect with me</Button></div></div></nav>
      {open && <div className="fixed inset-0 z-30 overflow-y-auto bg-black/95 px-6 pb-10 pt-24 backdrop-blur-xl lg:hidden">{navLinks.map(([label, href]) => <a onClick={() => setOpen(false)} key={label} href={href} className="block border-b border-white/10 py-5 text-[clamp(2rem,10vw,3rem)] font-black uppercase leading-none tracking-[-0.06em]">{label}</a>)}<div className="mt-7"><Button href="mailto:hemanthsairoyal7@gmail.com">Connect with me</Button></div></div>}
      <div className="relative z-10 mx-auto grid min-h-[100svh] max-w-7xl content-center gap-10 px-5 pb-10 pt-24 sm:px-7 sm:pb-12 sm:pt-28 lg:grid-cols-[1.04fr_0.96fr] lg:items-center lg:px-10"><div className="relative z-10"><FadeIn><p className="mb-4 flex flex-wrap items-center gap-2 text-[10px] uppercase tracking-[0.21em] text-white/60 sm:gap-3 sm:text-xs sm:tracking-[0.32em]"><Zap className="h-4 w-4 shrink-0" />AI / Software / Design / Writing / Discipline</p></FadeIn><motion.h1 initial={{ opacity: 0, y: 56 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, ease: [0.22, 1, 0.36, 1] }} className="max-w-[10ch] text-[clamp(3.5rem,16vw,8.15rem)] font-black uppercase leading-[0.82] tracking-[-0.085em] text-white">Building proof, not noise.</motion.h1><FadeIn delay={0.12}><p className="mt-5 max-w-xl text-base leading-relaxed text-white/72 sm:mt-7 sm:text-xl">I’m still early, but I’m building in public — turning curiosity across AI, software, design, and storytelling into visible proof.</p></FadeIn><FadeIn delay={0.2}><div className="mt-6 flex flex-wrap items-center gap-3 sm:mt-7"><span className="liquid-glass inline-flex max-w-full items-center gap-3 rounded-full px-4 py-3 text-[10px] uppercase tracking-[0.16em] text-white/70 sm:text-xs sm:tracking-[0.22em]"><span className="h-2 w-2 shrink-0 animate-pulse rounded-full bg-white" />Currently turning scattered ideas into visible projects</span></div><div className="mt-6 grid gap-3 sm:flex sm:flex-wrap sm:gap-4"><Button href="#work">See what I’m building</Button><Button href="mailto:hemanthsairoyal7@gmail.com">Connect with me</Button></div></FadeIn></div><FadeIn delay={0.08} className="relative mx-auto w-full max-w-[min(78vw,360px)] sm:max-w-[410px] lg:max-w-[500px]"><div className="absolute inset-x-6 bottom-2 top-16 rounded-full bg-white/10 blur-3xl" /><div className="relative rounded-[2.4rem] border border-white/10 bg-white/[0.035] p-3 shadow-[0_40px_140px_rgba(0,0,0,0.5)] sm:rounded-[3.2rem] sm:p-4"><ImageFrame src={images.hero} alt="Hemanth Sai outdoor portrait" label="Hero portrait" priority fit="cover" tone="natural" className="relative aspect-[4/5] rounded-[1.8rem] border border-white/10 bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.08),transparent_58%)] sm:rounded-[2.5rem]" imgClassName="object-[43%_50%]" /></div></FadeIn></div>
    </section>
  );
}
...
function Pressure() {
  return <section id="pressure" className="overflow-hidden bg-black px-5 py-16 text-white sm:px-8 sm:py-24 md:px-10"><div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-center"><FadeIn className="order-2 lg:order-1"><p className="mb-4 text-xs uppercase tracking-[0.3em] text-white/55 sm:mb-5 sm:tracking-[0.34em]">Forged under pressure</p><h2 className="max-w-5xl text-[clamp(3rem,13vw,8.2rem)] font-black uppercase leading-[0.84] tracking-[-0.08em]">Execution reveals character.</h2><p className="mt-5 max-w-2xl text-base leading-relaxed text-white/68 sm:mt-6 sm:text-lg">Pressure has been one of my oldest teachers — from sports and martial arts to leadership and building. I trust repetition, feedback, and performance more than empty motivation.</p></FadeIn><FadeIn delay={0.08} className="order-1 lg:order-2"><ImageFrame src={images.shotput} alt="Shotput action" label="Shotput performance" tone="soft" className="aspect-[16/10] rounded-[2rem] border border-white/10 shadow-[0_50px_140px_rgba(0,0,0,0.55)] sm:aspect-[16/9] sm:rounded-[3rem] lg:aspect-[16/10]" imgClassName="object-[52%_center]" /></FadeIn></div></section>;
}

function Proof() {
  return <section id="proof" className="bg-white px-5 py-16 text-black sm:px-8 sm:py-24 md:px-10"><div className="mx-auto max-w-7xl"><FadeIn><SectionLabel>Proof dashboard</SectionLabel><SectionHeading>Credibility without resume noise.</SectionHeading></FadeIn><div className="mt-10 grid gap-4 sm:mt-14 md:grid-cols-2 lg:grid-cols-3">{dashboard.map(([title, text], index) => <FadeIn key={title} delay={index * 0.035}><div className="min-h-[180px] rounded-[1.5rem] border border-black/10 bg-black/[0.035] p-5 sm:min-h-[220px] sm:rounded-[2rem] sm:p-7"><p className="mb-6 text-xs uppercase tracking-[0.28em] text-black/35 sm:mb-8">0{index + 1}</p><h3 className="mb-3 text-xl font-black uppercase tracking-[-0.05em] sm:text-2xl">{title}</h3><p className="text-sm leading-relaxed text-black/60 sm:text-base">{text}</p></div></FadeIn>)}</div></div></section>;
}

function System() {
  return <section id="system" className="bg-[#050505] px-5 py-16 text-white sm:px-8 sm:py-24 md:px-10"><div className="mx-auto max-w-7xl"><FadeIn><h2 className="text-[18vw] font-black uppercase leading-none tracking-[-0.09em] text-white/10 sm:text-[14vw] lg:text-[8rem]">The System</h2><p className="mt-5 max-w-3xl text-xl font-light leading-relaxed text-white/75 sm:mt-6 sm:text-2xl">I learn best through building. A project exposes the problem, the problem forces the skill, and the artifact gives me something real to improve.</p></FadeIn><div className="hide-scrollbar mt-10 flex snap-x gap-4 overflow-x-auto pb-4 sm:mt-14">{systemSteps.map((step, index) => <div key={step} className="liquid-glass min-w-[210px] snap-start rounded-[1.5rem] p-6 sm:min-w-[260px] sm:rounded-[2rem] sm:p-7"><p className="text-5xl font-black text-white/15 sm:text-6xl">0{index + 1}</p><h3 className="mt-9 text-xl font-black uppercase tracking-[-0.05em] sm:mt-10 sm:text-2xl">{step}</h3></div>)}</div></div></section>;
}

function ProjectCard({ project, index }: { project: typeof projects[number]; index: number }) {
  const [number, name, type, description, tags] = project;
  return <FadeIn delay={index * 0.06}><article className="group overflow-hidden rounded-[1.75rem] border border-white/10 bg-[#090909] p-5 shadow-[0_30px_90px_rgba(0,0,0,0.45)] transition duration-300 hover:-translate-y-1 hover:border-white/18 sm:rounded-[2.5rem] sm:p-7 lg:max-w-4xl lg:p-8"><div className="flex items-start justify-between gap-5"><span className="text-6xl font-black tracking-[-0.08em] text-white/13 sm:text-7xl">{number}</span><span className="liquid-glass rounded-full px-4 py-2 text-[10px] uppercase tracking-[0.22em] text-white/60">{type}</span></div><h3 className="mt-8 text-[clamp(2.1rem,9vw,4.1rem)] font-black uppercase leading-none tracking-[-0.07em]">{name}</h3><p className="mt-5 max-w-2xl text-base leading-relaxed text-white/64 sm:text-lg">{description}</p><div className="mt-7 flex flex-wrap gap-2.5 sm:gap-3">{tags.map((tag) => <span key={tag} className="rounded-full border border-white/10 px-3.5 py-2 text-[10px] uppercase tracking-[0.16em] text-white/55 sm:px-4 sm:text-xs sm:tracking-[0.2em]">{tag}</span>)}</div></article></FadeIn>;
}

function Work() {
  return <section id="work" className="bg-[#050505] px-5 py-16 text-white sm:px-8 sm:py-24 md:px-10"><div className="mx-auto max-w-7xl"><FadeIn><SectionLabel>Work in progress</SectionLabel><h2 className="text-[18vw] font-black uppercase leading-none tracking-[-0.09em] text-white/10 sm:text-[14vw] lg:text-[8rem]">Taking Shape</h2><p className="mt-5 max-w-3xl text-xl font-light leading-relaxed text-white/72 sm:text-2xl">I’m not showing a fake wall of finished products. These are the initiatives I’m actively trying to turn into visible proof.</p></FadeIn><div className="mt-10 grid gap-5 lg:mt-14 lg:gap-7">{projects.map((project, index) => <ProjectCard key={project[1]} project={project} index={index} />)}</div></div></section>;
}

function Capabilities() {
  return <section className="bg-white px-5 py-16 text-black sm:px-8 sm:py-24 md:px-10"><div className="mx-auto max-w-7xl"><FadeIn><SectionHeading>Capability pillars.</SectionHeading></FadeIn><div className="mt-10 grid gap-4 sm:mt-14 sm:grid-cols-2 lg:grid-cols-5">{pillars.map(([number, title, text, Icon]) => <div key={title} className="rounded-[1.5rem] border border-black/10 bg-black/[0.035] p-5 sm:rounded-[2rem] sm:p-6"><Icon className="mb-7 h-8 w-8 sm:mb-10" /><p className="text-sm text-black/35">{number}</p><h3 className="mt-4 text-lg font-black uppercase tracking-[-0.04em] sm:text-xl">{title}</h3><p className="mt-4 text-sm leading-relaxed text-black/60">{text}</p></div>)}</div></div></section>;
}

function Obsessions() {
  return <section className="bg-[#050505] px-5 py-16 text-white sm:px-8 sm:py-24 md:px-10"><div className="mx-auto max-w-7xl"><FadeIn><SectionLabel>Current explorations</SectionLabel><SectionHeading light>The ideas I keep returning to.</SectionHeading></FadeIn><div className="mt-10 grid gap-3.5 sm:mt-14 md:grid-cols-2">{obsessions.map((obsession, index) => <FadeIn key={obsession} delay={index * 0.03}><div className="liquid-glass flex min-h-16 items-center justify-between gap-5 rounded-[1.5rem] px-5 py-5 sm:rounded-[2rem] sm:px-6 sm:py-6"><span className="text-xl font-semibold tracking-[-0.05em] sm:text-2xl">{obsession}</span><span className="shrink-0 text-[10px] uppercase tracking-[0.18em] text-white/35 sm:text-xs sm:tracking-[0.24em]">Active</span></div></FadeIn>)}</div></div></section>;
}

function Evidence() {
  const evidence = [
    ['Karate roots', 'Karate gave me my first relationship with repetition, discipline, and physical confidence.', images.martial, 'Karate training collage', 'Karate roots', 'md:col-span-2 h-[390px] sm:h-[520px]', 'object-[50%_50%]', 'cover'],
    ['Voice', 'Public speaking turned ideas into presence.', images.stage, 'Stage speaking', 'Stage speaking', 'md:col-span-3 h-[280px] sm:h-[360px]', 'object-[50%_34%]', 'cover'],
    ['Pressure', 'Sport taught me that execution reveals character.', images.shotput, 'Shotput action', 'Shotput', 'md:col-span-5 h-[340px] sm:h-[520px]', 'object-[50%_center] p-2 sm:p-4', 'contain'],
  ] as const;
  return <section className="bg-white px-5 py-16 text-black sm:px-8 sm:py-24 md:px-10"><div className="mx-auto max-w-7xl"><FadeIn><SectionHeading>Proof is not always digital.</SectionHeading><p className="mt-5 max-w-2xl text-base leading-relaxed text-black/55 sm:mt-6 sm:text-lg">These images are not decoration. They are reminders of discipline, pressure, communication, and the person behind the screen.</p></FadeIn><div className="mt-10 grid gap-4 sm:mt-14 md:grid-cols-5">{evidence.map(([title, caption, src, alt, label, className, imgClassName, fit]) => <FadeIn key={title} className={className}><article className="group relative h-full overflow-hidden rounded-[1.5rem] border border-black/10 bg-black/[0.035] sm:rounded-[2rem]"><ImageFrame src={src} alt={alt} label={label} tone="soft" fit={fit} className="absolute inset-0 h-full w-full" imgClassName={imgClassName} /><div className="absolute inset-x-0 bottom-0 z-10 p-5 text-white sm:p-6"><p className="text-[10px] uppercase tracking-[0.22em] text-white/58">{title}</p><p className="mt-2 max-w-md text-sm leading-relaxed text-white/78 sm:text-base">{caption}</p></div></article></FadeIn>)}</div></div></section>;
}
